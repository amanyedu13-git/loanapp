...
# ── Computed Features ─────────────────────────────────────────
effective_loan     = loan_amount + existing_loans   # existing loans bhi model ko denge
loan_income_ratio  = effective_loan / income        # model ke liye
emi_income_ratio   = (effective_loan / 60) / income
income_per_person  = income / (dependents + 1)
total_loan_burden  = (loan_amount + existing_loans) / income  # scoring ke liye
...
if st.button("🔍 Predict Loan Approval"):
    ...
    # Total Loan Burden (10) — loan+existing / income
    if total_loan_burden < 0.5:
        score += 10; score_details.append(("Total Loan Burden", 10, 10, "✅"))
    elif total_loan_burden < 1.0:
        score += 5;  score_details.append(("Total Loan Burden", 5, 10, "🟡"))
    else:
        score_details.append(("Total Loan Burden", 0, 10, "❌"))  # >1x = critical
